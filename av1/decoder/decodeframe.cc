...
static TX_MODE read_tx_mode(struct aom_read_bit_buffer *rb) {
  ...
  return tx_mode;
#else
  return aom_rb_read_bit(rb) ? TX_MODE_SELECT :(TX_MODE)aom_rb_read_literal(rb, 2);
...
static PARTITION_TYPE read_partition(AV1_COMMON *cm, MACROBLOCKD *xd,
	...
	//здесь вычисляется индекс контекста в зависимости от соседей сверху и слева того же размера
...
static void decode_partition(AV1Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  //printf("\n r= %d c= %d bsize= %d partition= %d", mi_row, mi_col, bsize, partition);

  int length = 0;
  switch (bsize) {
	case      BLOCK_4X4  : length=4;     break;
	case	  BLOCK_4X8  : length=4;	 break;
	case	  BLOCK_8X4  : length=4;	 break;
	case	  BLOCK_8X8  : length=8;	 break;
	case	  BLOCK_8X16 : length=8;	 break;
	case	  BLOCK_16X8 : length=8;	 break;
	case	  BLOCK_16X16: length=16;	 break;
	case	  BLOCK_16X32: length=16;	 break;
	case	  BLOCK_32X16: length=16;	 break;
	case	  BLOCK_32X32: length=32;	 break;
	case	  BLOCK_32X64: length=32;	 break;
  ...
  if (seg->update_data) {
    ...
    for (i = 0; i < MAX_SEGMENTS; i++) {
      for (j = 0; j < SEG_LVL_MAX; j++) {
        ...
        if (feature_enabled) {
          av1_enable_segfeature(seg, i, (SEG_LVL_FEATURES)j);
          data = decode_unsigned_max(rb, av1_seg_feature_data_max((SEG_LVL_FEATURES)j));
          if (av1_is_segfeature_signed((SEG_LVL_FEATURES)j))
            data = aom_rb_read_bit(rb) ? -data : data;
        }
        av1_set_segdata(seg, i, (SEG_LVL_FEATURES)j, data);
      }
    }
  }
}
...
static void setup_frame_size(AV1_COMMON *cm, struct aom_read_bit_buffer *rb) {
  ...
  pool->frame_bufs[cm->new_fb_idx].buf.color_range = (aom_color_range_t)cm->color_range;
...
static void setup_frame_size_with_refs(AV1_COMMON *cm,
  ...
  for (i = 0; i < INTER_REFS_PER_FRAME; ++i) {
    RefBuffer *const ref_frame = &cm->frame_refs[i];
    if (!valid_ref_frame_img_fmt((aom_bit_depth_t)ref_frame->buf->bit_depth,
...
static const uint8_t *decode_tiles(AV1Decoder *pbi, const uint8_t *data,
  ...
  if (pbi->tile_data == NULL || n_tiles != pbi->allocated_tiles) {
    ...
    CHECK_MEM_ERROR(cm, pbi->tile_data,
                    (TileData *)aom_memalign(32, n_tiles * (sizeof(*pbi->tile_data))));
...
static const uint8_t *decode_tiles_mt(AV1Decoder *pbi, const uint8_t *data,
  ...
  if (pbi->num_tile_workers == 0) {
    ...
    CHECK_MEM_ERROR(cm, pbi->tile_workers,
		(AVxWorker *)aom_malloc(num_threads * sizeof(*pbi->tile_workers)));
    // Ensure tile data offsets will be properly aligned. This may fail on
    // platforms without DECLARE_ALIGNED().
    assert((sizeof(*pbi->tile_worker_data) % 16) == 0);
    CHECK_MEM_ERROR(
        cm, pbi->tile_worker_data,
        (TileWorkerData*)aom_memalign(32, num_threads * sizeof(*pbi->tile_worker_data)));
    CHECK_MEM_ERROR(cm, pbi->tile_worker_info,
                    (TileInfo *)aom_malloc(num_threads * sizeof(*pbi->tile_worker_info)));
...
static void read_bitdepth_colorspace_sampling(AV1_COMMON *cm,
  ...
  cm->color_space = (aom_color_space_t)aom_rb_read_literal(rb, 3);
...
static size_t read_uncompressed_header(AV1Decoder *pbi,
  ...
  get_frame_new_buffer(cm)->color_range = (aom_color_range_t)cm->color_range;
